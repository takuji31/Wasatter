...
public class TaskImageDownloadWithCache extends AsyncTask<Void, Integer, Void> {
    ...
    protected void onPreExecute() {
        ...
        if (count != 0) {
            progress_text_count.setText(new SpannableStringBuilder("0/")
    ...
    protected void onProgressUpdate(Integer... values) {
        ...
        SpannableStringBuilder sb = new SpannableStringBuilder(
                String.valueOf(Wasatter.main.progress_image.getProgress()))
                .append("/").append(String.valueOf(count));
        if (this.errorCount != 0) {
            sb.append(" (").append(String.valueOf(this.errorCount))
                    .append(" Error)");
        }
        progress_text_count.setText(sb.toString());
        try {
            Timeline adapter = (Timeline) Wasatter.main.ls.getAdapter();
            if (adapter != null) {
                adapter.updateView();
            }
